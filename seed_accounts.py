...
def seed_accounts():
    ...
    # ----------------------------------------------------
    # NEW, VALID TELETHON SESSIONS PROVIDED BY USER
    # ----------------------------------------------------
    accounts_to_add = [
        ...
        ("918217240245", "1BVtsOHoBu2j-DYrJTHKwa_vhHchzm497nXNhKuRMvjPZlnCgbB5T8TF4nzHc2fao5fsVDlpMD5tjIGp6Kh6IRAlNIagwMPtyUv2BgFTax3FFqumvV6JH6lZZtDk6MehaOj0JOlbWlKOTe1t__-pGjW3zCyNXKTQD1blTqQwFEQGyINQCz99mh6BthuF27XtF-nOOa2pKA68IEyTlLd_ilabLMBJIMd0PPiv34hw3r5IwRo_JMoFhFJmzOHybQYYVKLRNJL7FvsAVJrL3UeCzh4gbNNzGaCxysvpvkkpaK83FcXWVoVI9P54qfPI_x9kGpBLZT7BgO7aubCnfqQ2T0dHi3_EbAgQ="),
        ...
        ("918281592533", "1BVtsOHoBux8yBCgA8mp0Y12_aCmVcgp7nxKlYHx93Gi7BTiEaT56ts_KTslhwU4xGPz0M7SnBxy-jdYr7G7truiIDYW2b8O1FQbYtvql151cJk4ZLfx4plBbXyGD4e_No_E_RrpBgk24UrRb42ZbEj-MC8snEB9Iv5M46LdKOksoYBENpxVOSr9AgqmVmmYaestW-2BxKFB1WPJCsm9W1PAWxOLGiPqW_-8Sscbz9GNBFJEnYFeEA2a7ZgBB30LENnBzm25-x7h79pBwGyCW3JXtog65sbzkebfE-oFaSeKAep4QbB4X2qXbWm7dywkdEQpNiPuP93bwfx4C49dPidAww6vPiRak=")
    ]

    print(f"⚠️ Clearing {db.get_account_pool_stats().get('total_accounts')} existing accounts...")
    # Clear all accounts to remove any old, invalid sessions
    db.connection.cursor().execute("DELETE FROM sold_accounts;")
    db.connection.commit()
    print("✅ Existing accounts cleared.")
    
    print(f"🚀 Starting bulk import of {len(accounts_to_add)} Telethon accounts...")
    
    success_count = 0
    for phone, session in accounts_to_add:
        try:
            # Use add_account_manual (which uses seller_user_id=0 and status='active')
            account_id = db.add_account_manual(phone, session.strip())
